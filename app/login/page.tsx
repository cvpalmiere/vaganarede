    ...
    <div className="min-h-screen bg-deep-black text-white flex items-center justify-center px-6">
      <div className="glass-dark p-8 md:p-10 rounded-card border border-gray-800 w-full max-w-md bg-[#18181b]">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {erro && <p className="text-sm text-red-400 bg-red-950/50 p-3 rounded-lg border border-red-800">{erro}</p>}

          <button
            type="submit"
            disabled={carregando}
            className="w-full bg-electric-yellow text-deep-black font-bold py-4 rounded-lg hover:opacity-90 transition disabled:opacity-50"
          >
        ...
        >
          ...
          Ainda não tem conta? Fale com a gente
        </Link>
      </div>
    </div>
  );
